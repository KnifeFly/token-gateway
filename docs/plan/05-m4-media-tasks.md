...
支持网剧 Agent 和多模态工作流需要的生图、生视频、音频、音乐、文件上传、异步任务、任务查询和 callback。
...
3. 请求通过 price precheck 和 balance hold 后创建 internal task。
4. ProviderTaskDispatcher 提交外部任务并落库 external_task_id。
5. worker 通过 polling 或 webhook 更新任务状态。
6. Result normalizer 把 provider 结果转为统一资产。
7. 任务完成后结算并通过 callback outbox 通知客户。
...
- [实施计划 M4](../design/ai_gateway_implementation_plan_v2.md)
- [系统设计异步媒体任务](../design/ai_gateway_system_design_v2.md)
- [架构设计异步任务架构](../design/ai_gateway_architecture_design_v2.md)
- [OpenAPI 草案媒体和任务接口](../design/ai_gateway_openapi_v2.yaml)
- [任务清单 Epic 11](../design/ai_gateway_task_list_v2.md)
