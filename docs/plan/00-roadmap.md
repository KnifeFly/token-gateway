...
把 `docs/design` 中的系统设计、架构设计、代码蓝图和任务清单整理成可执行路线图。推进顺序遵循“先数据面、后控制面；先账务闭环、后商业扩展；先内置插件、后动态扩展”的原则。
...
| v1.0 commercial launch | M8 | 支持正式商业运营、对账、报表和模型市场 |

## 交付物

- `docs/plan/01-m0-foundation.md` 到 `docs/plan/09-m8-commercial-ops.md` 作为阶段规划。
- `docs/tasks.md` 作为任务看板和执行入口。
- 阶段文档只沉淀执行化摘要，设计真相仍以 `docs/design` 为准。
...
9. M8 补齐 usage/cost/profit report、账单导出、对账和运营后台能力。
...
- M0-M8 的先后关系与设计包一致。
- 所有 public API 变更都回到 OpenAPI 草案维护。
...
- [设计包索引](../design/ai_gateway_design_pack_v2_README.md)
- [系统设计](../design/ai_gateway_system_design_v2.md)
- [架构设计](../design/ai_gateway_architecture_design_v2.md)
- [代码蓝图](../design/ai_gateway_code_blueprint_v2.md)
- [实施计划](../design/ai_gateway_implementation_plan_v2.md)
- [任务清单](../design/ai_gateway_task_list_v2.md)
