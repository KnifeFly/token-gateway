...
- P22 进展：Console Frontend Production 已落地，覆盖 Admin 生产化 app shell、session restore/login/logout、生产工作台、危险操作 UX、shared API client CSRF/reason/idempotency、Portal/Admin 安全头、静态资源 cache policy、Docker/compose console 交付、`p22-console-smoke`、release handoff 证据、production runbook 和 Browser 桌面/窄屏 smoke。
...
- P24 进展：NewAPI Lean Console Parity 设计与计划已落地，已完成 Admin/Portal P24 信息架构、route plan、中文展示文案入口和 i18n 预留；已完成 Channel Management lean workflows，覆盖 channel safe CRUD、credential rotation、test、sync preview/apply、health read model、model coverage、route policy hint、audit 和 Admin 中文渠道面板；已完成 Model Management 和 Portal 模型预览，覆盖 model safe catalog、schema preview、component pricing summary、channel coverage、catalog sync preview、Admin 中文模型面板和 Portal 模型详情预览；已完成 Customer Account Management，覆盖 tenant/project scoped account、状态、角色、额度、API key 元数据、Portal 会话重置、人工调账、审计和 Admin 中文客户账户面板；已完成 Admin/Portal API Key Management，覆盖 create/disable/enable/rotate、allowed models、IP allowlist、expires_at、usage summary、一次性明文展示和 Admin/Portal 中文密钥面板；已完成 Usage Logs 和 Task Logs 运营视图，覆盖 Admin/Portal request_id/task_id drilldown、tenant/project/API key/model/channel/provider/status/time range 筛选、safe metadata 详情和中文活动日志页面；已完成 schema-driven Playground 和 Channel Test 复用，覆盖 Admin/Portal scope 隔离、结构化 payload、安全调试结果、导入导出脱敏和中文操练场展示；已完成最小额度运营和 ledger/export 视图，覆盖 Admin customer credit report、usage/ledger safe export、Portal ledger/export、active holds 摘要、failed settlement links、人工调整约束和中文额度运营面板；已完成 OpenAPI/generated client/contract tests 收口，覆盖 P24 BFF path、mutation security、safe DTO denylist、Portal task safe schema 和 generated client drift；已完成 cut-scope regression，覆盖 OpenAPI route/schema、generated client、前端 route config、API call 和裁剪范围外 feature 文件静态检查；已完成 P24 console smoke、runbook 和 release evidence，覆盖 Admin/Portal 关键 workflow、`p24_console_smoke=passed` 证据、known limits 和 handoff 字段；只补齐渠道、模型、用户/客户账户、令牌、日志、任务、操练场和最小额度运营，用户分组、模型分组、渠道分组、倍率、订阅、兑换码、支付、邀请返利、模型部署和复杂系统设置不进入 P24。
- P25 进展：Console Frontend Data Connection and Routing Hardening 已落地，覆盖 Admin mutation shared wrapper、Admin 渠道/模型/客户账户/API key/活动日志/额度运营/操练场真实 BFF 数据连通、Portal route/popstate 和 usage/tasks/ledger limit 状态、shared UI primitives、SiliconFlow-like 轻量云控制台视觉对齐、runbook、release handoff 字段和 Browser smoke；不新增 NewAPI 裁剪外模块。
- 待执行：复杂财务/发票闭环、对象存储、完整 Realtime、生产级 Observability 扩展、WASM/动态插件、semantic routing/cache 和多地域 active-active 先不做；如需要对象存储能力，应另立独立阶段。
- 阻塞：无。
- 下一步建议：P25 已收口；后续如继续 console 路线，应另立 P26，并先明确是否进入更完整运营分析、对象存储或生产 Observability，而不是在 P25 内扩大范围。
- 本次开发验证：2026-06-07 P25 Console Frontend Data Connection and Routing Hardening `pnpm --filter @token-gateway/ui typecheck`、`pnpm --filter @token-gateway/admin typecheck`、`pnpm --filter @token-gateway/portal typecheck`、`pnpm typecheck`、`pnpm lint`、`pnpm test`、`pnpm build`、`make p24-cut-scope-check`、`make boundary-check`、`make api-check`、`go test ./internal/app/admin/... ./internal/transport/adminhttp ./internal/app/portal/... ./internal/transport/portalwebhttp ./tests/contract ./tools/release-handoff`、`go run ./tools/p24-console-smoke -console-url http://127.0.0.1:9515 -api-key tg-local-dev-key -admin-email admin@example.com -admin-password admin-local -model gpt-4o-mini -create-derived-key`、`git diff --check`、trailing-whitespace scan、Admin sample data scan、Admin wrapper bypass scan 和 Browser Admin/Portal route/list/shared UI/SiliconFlow-like style smoke 通过；no-DB console 使用本地 seed snapshot，DB/Redis/billing/limits 关闭。
- 本次规划验证：2026-06-07 P25 Console Frontend Data Connection and Routing Hardening 设计、计划、SiliconFlow-like 视觉对齐要求和 tasks 落库后，`git diff --check` 与 markdown trailing-whitespace scan 通过；本次为 docs-only 变更，未运行代码测试。2026-06-02 P24 NewAPI Lean Console Parity 设计、计划、roadmap、tasks、CLAUDE 和 design index 落库后，`git diff --check` 与 markdown trailing-whitespace scan 通过；本次为 docs-only 变更，未运行代码测试。2026-06-02 P23 Console Directory Structure Alignment 计划文档落库并接入 roadmap/tasks/design/CLAUDE 后，`git diff --check` 与 markdown trailing-whitespace scan 通过；本次为 docs-only 变更，未运行代码测试。2026-06-02 P23 Portal 迁移目标调整为全量迁移并删除 `internal/portal` 运行包后，`git diff --check` 与 markdown trailing-whitespace scan 通过；本次为 docs-only 变更，未运行代码测试。2026-06-01 Portal/Admin Console 设计与 P19-P22 计划文档落库后，`git diff --check` 与 markdown trailing-whitespace scan 通过；当次为 docs-only 变更，未运行代码测试。
...
- 本次开发验证：2026-06-02 P24 Model Management 和 Portal 模型预览 `go test ./internal/app/admin/... ./internal/app/portal/... ./internal/transport/adminhttp ./internal/transport/portalwebhttp ./internal/controlplane/configadmin ./tests/contract`、`pnpm typecheck`、`pnpm build`、`pnpm test`、`pnpm lint`、`pnpm generate:api`、`api/scripts/lint_openapi.sh`、`make api-check`、frontend/API cut-scope scan、Browser Admin 模型面板 smoke 和 Portal 登录后模型预览 smoke 通过。
- 本次开发验证：2026-06-02 P24 Customer Account Management `go test ./internal/app/admin/... ./internal/app/portal/... ./internal/transport/adminhttp ./internal/transport/portalwebhttp ./internal/controlplane/configadmin ./tests/contract ./internal/bootstrap`、`pnpm typecheck`、`pnpm build`、`pnpm test`、`pnpm lint`、`pnpm generate:api`、`api/scripts/lint_openapi.sh`、`git diff --check`、trailing-whitespace scan、customer account denylist scan、Browser Admin 客户账户桌面/移动 smoke 通过；提交后需重跑 `make api-check` 确认 generated client 无 drift。
- 本次开发验证：2026-06-02 P24 Admin/Portal API Key Management `go test ./internal/controlplane/configadmin ./internal/controlplane/snapshot ./internal/dataplane/auth ./internal/dataplane/snapshot ./internal/app/admin/... ./internal/app/portal/... ./internal/transport/adminhttp ./internal/transport/portalwebhttp ./internal/billing/reporting ./tests/contract ./internal/bootstrap`、`pnpm generate:api`、`api/scripts/lint_openapi.sh`、`pnpm typecheck`、`pnpm build`、`pnpm test`、`pnpm lint`、`make migrate-up`、`go run ./tools/portal-web-smoke -console-url http://127.0.0.1:9505 -api-key tg-local-dev-key -create-derived-key`、Browser Admin/Portal API Keys 桌面/移动 smoke、`git diff --check`、trailing-whitespace scan 和 API key cut-scope/secret scan 通过；提交后需重跑 `make api-check` 确认 generated client 无 drift。
- 本次开发验证：2026-06-02 P24 Usage Logs 和 Task Logs 运营视图 `go test ./internal/billing/reporting ./internal/task ./internal/app/admin/... ./internal/app/portal/... ./internal/transport/adminhttp ./internal/transport/portalwebhttp ./internal/transport/portalhttp ./tests/contract ./internal/bootstrap`、`pnpm generate:api`、`api/scripts/lint_openapi.sh`、`pnpm typecheck`、`pnpm build`、`pnpm test`、`pnpm lint`、`git diff --check`、trailing-whitespace scan、cut-scope/secret scan、Browser Admin 活动日志 smoke、Browser Portal 用量/任务筛选 smoke 和提交后 `make api-check` 通过；Browser screenshot capture 在当前 in-app browser 中超时，功能状态以 DOM/console/交互检查为准。
- 本次开发验证：2026-06-02 P24 schema-driven Playground 和 Channel Test 复用 `go test ./internal/app/admin/... ./internal/app/portal/... ./internal/transport/adminhttp ./internal/transport/portalwebhttp ./tests/contract ./internal/bootstrap`、`pnpm generate:api`、`api/scripts/lint_openapi.sh`、`pnpm typecheck`、`pnpm build`、`pnpm test`、`pnpm lint`、`git diff --check`、trailing-whitespace scan、cut-scope/secret scan、Browser Admin 工具面板 smoke、Browser Portal 操练场运行 smoke 和提交后 `make api-check` 通过。
- 本次开发验证：2026-06-02 P24 最小额度运营和 ledger/export 视图 `go test ./internal/app/admin/... ./internal/app/portal/... ./internal/transport/adminhttp ./internal/transport/portalwebhttp ./tests/contract ./internal/bootstrap`、`pnpm generate:api`、`api/scripts/lint_openapi.sh`、`pnpm typecheck`、`pnpm build`、`pnpm test`、`pnpm lint`、`git diff --check`、trailing-whitespace scan、cut-scope/secret scan、Browser Admin 额度运营 smoke、Browser Portal 额度页账本/导出 smoke 和提交后 `make api-check` 通过。
- 本次开发验证：2026-06-02 P24 OpenAPI/generated client 和 contract tests 收口 `go test ./internal/app/admin/... ./internal/app/portal/... ./internal/transport/adminhttp ./internal/transport/portalwebhttp ./tests/contract ./internal/bootstrap`、`pnpm generate:api`、`api/scripts/lint_openapi.sh`、`pnpm typecheck`、`pnpm build`、`pnpm test`、`pnpm lint`、`git diff --check` 和提交后 `make api-check` 通过。
- 本次开发验证：2026-06-02 P24 cut-scope regression `go test ./tests/contract`、`make p24-cut-scope-check`、`make boundary-check`、`git diff --check` 和 trailing-whitespace scan 通过。
- 本次开发验证：2026-06-02 P24 E2E smoke、runbook 和 release evidence `go test ./tools/release-handoff ./tools/p24-console-smoke ./tools/portal-web-smoke ./tests/contract`、`make p24-cut-scope-check`、`go run ./tools/release-handoff -timeout 30s`、本地 no-DB console `go run ./tools/p24-console-smoke -console-url http://127.0.0.1:9515 -api-key tg-local-dev-key -admin-email admin@example.com -admin-password admin-local -model gpt-4o-mini -create-derived-key`、`git diff --check` 和 trailing-whitespace scan 通过。
- 本次开发验证：2026-06-07 P22 Console Frontend Production `go test ./internal/bootstrap ./internal/app/admin/service ./internal/transport/adminhttp ./internal/transport/consolehttp ./tools/p22-console-smoke`、`go test ./...`、`go vet ./...`、`go build ./cmd/...`、`pnpm typecheck`、`pnpm lint`、`pnpm test`、`pnpm build`、`make boundary-check`、`make api-check`、`docker compose config`、`go run ./tools/p22-console-smoke -console-url http://127.0.0.1:9515 -api-key tg-local-dev-key -admin-email admin@example.com -admin-password admin-local` 和 Browser Admin 工作台/渠道页/validate snapshot/390px 窄屏 smoke 通过；no-DB console 使用本地 seed snapshot，DB/Redis/billing/limits 关闭。
...
- P19-P25 console full-stack、结构治理、NewAPI 精简对标与前端数据连通任务 ID 采用 `P{phase}/E{epic}-T{number}/P{priority}` 格式，P19-P22 是产品/生产化阶段，P23 是行为保持型目录结构对齐阶段，P24 是 NewAPI lean parity 产品补齐阶段，P25 是 review 驱动的 Admin/Portal 真实数据连通与路由/列表硬化阶段。
...
- P25 只做已进入 P24 范围的 Admin/Portal 前端真实 BFF 连通、Admin mutation 安全约束、Portal/App 拆分、browser history、最小 shared UI primitives 和 SiliconFlow-like 轻量云控制台视觉对齐；不借 P25 引入裁剪外模块、完整路由框架、全局状态库、Storybook、图表体系、暗色主题，也不复制 SiliconFlow logo、品牌素材或文案。
...
| 状态 | ID | 优先级 | 任务 | 目标位置 | 验收标准 |
|---|---|---|---|---|---|
| [x] | P22/E32-T01/P0 | P0 | 实现 Admin frontend app shell 和核心页面 | `web/apps/admin`, `web/packages/ui`, `web/packages/auth` | dashboard、tenants、projects、api-keys、models、channels、routes、pricing、limits、snapshots、operations、audit、operators 可用 |
| [x] | P22/E32-T02/P0 | P0 | 实现危险操作 UX 和权限体验 | `web/apps/admin`, `web/packages/auth` | confirm、reason、diff/dry-run、idempotency、permission denied、session expired 行为稳定 |
| [x] | P22/E32-T03/P0 | P0 | 收口 Portal/Admin shared frontend packages | `web/packages/ui`, `web/packages/api-client`, `web/packages/format` | shared packages 只放纯 UI/auth/format/client 能力，不包含业务 feature 组件 |
| [x] | P22/E32-T04/P0 | P0 | 建立 Portal/Admin E2E smoke | Playwright or equivalent, tests/e2e, CI | Portal login/key/usage/tasks/logout 和 Admin login/config/snapshot/audit/logout 可重复执行 |
| [x] | P22/E32-T05/P1 | P1 | 固化 static asset 和 security headers | `cmd/console`, deploy configs, docs/runbook | `/portal/*`、`/admin-ui/*` cache policy、CSP、HSTS、cookie Secure/SameSite、CORS/CSRF 可验证 |
| [x] | P22/E32-T06/P1 | P1 | 更新 Docker/deploy/release gate | Dockerfile, deployments, Makefile, docs/runbook, release handoff | Go checks、OpenAPI checks、web checks、E2E smoke、rollback/cache purge 进入发布证据 |
| [x] | P22/E32-T07/P1 | P1 | 补齐 console production runbook | `docs/runbook`, `docs/tasks.md`, `docs/plan/33-p22-console-frontend-production.md` | 本地、staging、production、operator bootstrap、session secret rotation、rollback 流程清晰 |
...
| [x] | P24/E34-T04/P0 | P0 | 实现 Model Management 和 Portal 模型预览 | `internal/app/admin/service`, `internal/app/portal/service`, `web/apps/admin`, `web/apps/portal` | model catalog metadata、category、modalities、capabilities、schema preview、pricing summary、channel coverage 和 Portal preview 可用，UI/API 不出现倍率 |
| [x] | P24/E34-T05/P0 | P0 | 实现 Customer Account Management | `internal/app/admin/service`, `internal/app/admin/repository`, `internal/billing`, `web/apps/admin` | tenant/project scoped customer account、status、role、credits、API key、session reset、manual adjustment 和 audit 可用，UI/API 不出现用户分组 |
| [x] | P24/E34-T06/P0 | P0 | 收口 Admin/Portal API Key 管理 | `internal/app/admin/service`, `internal/app/portal/service`, `api/openapi`, `web/apps/admin`, `web/apps/portal` | create/disable/rotate、allowed models、IP allowlist、expires_at、usage summary 可用，plaintext key 只显示一次且不能扩大权限 |
| [x] | P24/E34-T07/P1 | P1 | 实现 Usage Logs 和 Task Logs 运营视图 | `internal/app/admin/repository`, `internal/app/portal/repository`, `web/apps/admin`, `web/apps/portal` | usage/task 支持 time range、request_id/task_id、tenant/project、API key、model、channel、provider、status 等筛选，详情只返回 safe metadata |
| [x] | P24/E34-T08/P1 | P1 | 实现 schema-driven Playground 和 Channel Test 复用 | `internal/app/admin/service`, `internal/app/portal/service`, `web/apps/admin`, `web/apps/portal` | Admin/Portal playground scope 隔离，支持 schema-driven payload、stream/debug safe result、导入导出不含 secret，并复用 channel test executor |
| [x] | P24/E34-T09/P1 | P1 | 实现最小额度运营和 ledger/export 视图 | `internal/billing`, `internal/app/admin/service`, `internal/app/portal/service`, `web/apps/admin`, `web/apps/portal` | balance、ledger、active holds、failed settlement links、manual adjustment 和 usage/ledger export 可用，额度调整必须 reason + idempotency + audit |
| [x] | P24/E34-T10/P1 | P1 | 补齐 P24 OpenAPI、generated client 和 contract tests | `api/openapi/admin-bff.yaml`, `api/openapi/portal-bff.yaml`, `web/packages/api-client`, tests | OpenAPI schema、generated TS client、BFF contract tests、RBAC/CSRF/audit/redaction tests 一致，denylist 字段不出现在 safe DTO |
| [x] | P24/E34-T11/P1 | P1 | 增加 P24 cut-scope regression | tests, `web/apps/admin`, `web/apps/portal`, docs/runbook | UI/API 不暴露 group/ratio/payment/subscription/redemption/deployment/broad settings route 或字段，任务板裁剪边界可被测试或静态检查覆盖 |
| [x] | P24/E34-T12/P1 | P1 | 完成 P24 E2E smoke、runbook 和 release evidence | tests/e2e or equivalent, docs/runbook, release handoff | Admin channel/model/customer/API key/log/playground smoke 和 Portal models/playground/API key/usage/tasks/credits smoke 可重复执行，release handoff 记录 known limits 和验证命令 |

## P25 Console Frontend Data Connection and Routing Hardening

| 状态 | ID | 优先级 | 任务 | 目标位置 | 验收标准 |
|---|---|---|---|---|---|
| [x] | P25/E35-T01/P0 | P0 | 固化 Admin mutation client contract | `web/apps/admin/src/shared/api`, `web/apps/admin/src/features/*/*Api.ts`, tests | 所有 Admin 写操作经过 shared wrapper 或等价 feature wrapper，CSRF、reason、idempotency 和错误归一可测试 |
| [x] | P25/E35-T02/P0 | P0 | Channel Management 接入真实 BFF | `web/apps/admin/src/features/channels` | 列表、筛选、详情、test、rotate、sync preview/apply、enable/disable 使用 BFF，生产组件无 `sampleChannels` |
| [x] | P25/E35-T03/P0 | P0 | Model Management 接入真实 BFF | `web/apps/admin/src/features/models` | catalog、schema preview、coverage、sync preview、disable/deprecate 使用 BFF，生产组件无 `sampleModels` |
| [x] | P25/E35-T04/P0 | P0 | Customer Accounts 与 Credit Report 接入真实 BFF | `web/apps/admin/src/features/accounts`, `web/apps/admin/src/features/credits` | 账户、API key summary、session reset、manual adjustment、credit report、ledger/export 联动真实 BFF，生产组件无账户和额度 sample data |
| [x] | P25/E35-T05/P0 | P0 | API Key Management 接入真实 BFF | `web/apps/admin/src/features/api-keys` | create/update/enable/disable/rotate、一次性明文展示和权限边界走 BFF，生产组件无 `sampleAPIKeys` |
| [x] | P25/E35-T06/P1 | P1 | Activity Logs 接入真实 BFF | `web/apps/admin/src/features/activity` | usage/task filters、cursor/limit、详情 safe metadata 和复制入口可用，生产组件无日志 sample data |
| [x] | P25/E35-T07/P1 | P1 | Tools Playground 接入真实 BFF | `web/apps/admin/src/features/tools` | run、import preview、export 使用 BFF，payload/result 脱敏，生产组件无 `sampleResult` |
| [x] | P25/E35-T08/P1 | P1 | 抽取最小 shared UI primitives | `web/packages/ui`, Admin/Portal features | DataTable、Pagination、FilterBar、ConfirmDialog、Drawer/Modal、Toast、Skeleton、CopyButton 被真实页面复用并有 focused tests |
| [x] | P25/E35-T09/P1 | P1 | Portal/Admin SiliconFlow-like 视觉风格对齐 | `web/packages/ui`, `web/apps/admin/src/styles.css`, `web/apps/portal/src/styles.css`, feature pages | 两端共享 console theme tokens，使用轻色 200px sidebar、紫色 primary、compact cards/tables/tags/filter controls、8px card/button radius 和低阴影；不复制 SiliconFlow logo、品牌素材或文案 |
| [x] | P25/E35-T10/P1 | P1 | Admin/Portal route 与 popstate 收口 | `web/apps/admin/src/app`, `web/apps/portal/src/app` | 直接访问子路由、刷新、浏览器 back/forward 均恢复正确 view，不散落 pathname 字符串 |
| [x] | P25/E35-T11/P1 | P1 | Portal App 拆分与列表分页硬化 | `web/apps/portal/src/app`, `web/apps/portal/src/features` | `App.tsx` 只做入口装配，session/route/feature hooks 拆分；usage/tasks/ledger 有分页、loading、error、empty state |
| [x] | P25/E35-T12/P1 | P1 | P25 cut-scope、API drift 和 regression tests | tests, `api/openapi`, `web` | `make p24-cut-scope-check` 覆盖 P25 前端路径；`make api-check`、frontend lint/typecheck/test/build 通过 |
| [x] | P25/E35-T13/P1 | P1 | P25 smoke、runbook 和 release evidence | tools, `docs/runbook`, release handoff | Admin 真实数据 workflow、Portal route/list workflow 和 Portal/Admin 视觉对齐截图 smoke 可重复执行，handoff 记录 known limits 和验证命令 |
...
| P25 | Console frontend data connection 可验证，Admin 渠道、模型、客户账户、API keys、活动日志、额度运营和操练场全部使用真实 BFF，Portal/Admin route、popstate、列表分页、最小 shared UI primitives、SiliconFlow-like 轻量云控制台视觉对齐和 smoke 证据完成，且 P24 cut-scope 继续通过 |
