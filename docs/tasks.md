...
- 进行中：P4 发布候选与商用上线验收；P4/E14-T01 干净依赖环境 RC smoke 已通过。
- 阻塞：无。
- 下一步建议：继续 P4/E14-T02，补齐 worker balance hold reaper 和 reconciliation scheduled job，再进入真实 provider release channel、configd 生产分发、OpenAPI 管理面合同和灰度上线 runbook。
- 最近验证：2026-05-30 `tests/rc/clean_env_smoke.sh` 使用独立 Docker compose project/volume 和自动避让端口跑通 MySQL、Redis、migration、gateway、control-api、configd、worker、health/ready、snapshot publish/watch/rollback、gateway chat 和 metrics，输出 `rc_smoke=passed`；此前 `go test ./...`、`make lint`、`make build`、Redis 集成、failure drills 和 load test 均通过。
...
| [ ] | P4/E14-T02/P0 | P0 | 补齐 worker 生产运营 job | `internal/worker/jobs`, `internal/billing`, `internal/task`, `internal/bootstrap/worker.go` | failed settlement replay、provider task poller、callback dispatcher、balance hold reaper、reconciliation scheduled job 均有 lease、metrics、重试和 focused tests |
| [ ] | P4/E14-T03/P0 | P0 | 接入真实 provider release channel | `internal/provider`, `internal/task`, `configs`, `docs/runbook` | OpenAI/Claude/Gemini 真实 channel 最小回归通过，至少一个真实媒体 provider task 完成 submit、poll、result、callback、settlement |
| [ ] | P4/E14-T04/P1 | P1 | 生产化 configd snapshot 分发 | `cmd/configd`, `internal/controlplane/snapshot`, `internal/dataplane/snapshot`, `internal/infra/redis` | Redis active snapshot key/pubsub 或明确 fallback 模式可复现，gateway reload、checksum、rollback、stale policy 和 diagnostics 覆盖 |
| [ ] | P4/E14-T05/P1 | P1 | 补齐 OpenAPI 管理面合同 | `docs/design/ai_gateway_openapi.yaml`, `internal/transport/controlhttp`, contract tests | tenant/project/api-key/model/channel/route/price/limit/plugin/snapshot/emergency 管理接口与实现一致，关键请求/响应示例完整 |
| [ ] | P4/E14-T06/P1 | P1 | 建立发布级观测与安全验收 | `deployments/observability`, `pkg/redaction`, `tests/failure`, `tools/loadtest` | dashboard、alert、redaction audit、provider attempt、billing backlog、snapshot stale、worker job、Redis latency 和 SLO 预算均可验证 |
