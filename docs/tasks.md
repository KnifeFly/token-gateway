...
- 进行中：P4 发布候选与商用上线验收；P4/E14-T01 干净依赖环境 RC smoke、P4/E14-T02 worker 运营 job 已完成。
- 阻塞：无。
- 下一步建议：继续 P4/E14-T03，接入真实 provider release channel，再进入 configd 生产分发、OpenAPI 管理面合同和灰度上线 runbook。
- 最近验证：2026-05-30 `tests/rc/clean_env_smoke.sh` 使用独立 Docker compose project/volume 和自动避让端口跑通 MySQL、Redis、migration、gateway、control-api、configd、worker、health/ready、snapshot publish/watch/rollback、gateway chat 和 metrics，输出 `rc_smoke=passed`；`go test ./internal/billing ./internal/worker ./internal/worker/jobs ./internal/bootstrap` 通过；此前 `go test ./...`、`make lint`、`make build`、Redis 集成、failure drills 和 load test 均通过。
